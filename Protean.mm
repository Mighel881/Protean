...
+ (BOOL)canHandleTapForItem:(UIStatusBarItem*)item {
    ...
    if (type <= 32) {
        ...
        if (mode == 0) {
          ...
          HBLogDebug(@"[Protean] invalid IconTap action for system item: %d", mode);
        }
    } else {
        ...
        if ([ident hasPrefix:@"com.shade.protean-"]) {
...
+ (id)HandlerForTapOnItem:(UIStatusBarItem*)item {
    //HBLogDebug(@"[Protean] HandlerForTapOnItem");
    int type = MSHookIvar<int>(item, "_type");
    ...
    if (type <= 32) {
        ...
        if (mode == 0) {
          return nil;
            ...
            // Activator
            LASendEventWithName([NSString stringWithFormat:@"com.shade.protean-%@",ident]);
        } else {
          HBLogDebug(@"[Protean] invalid IconTap action for system item: %d", mode);
        }
    } else {
        ...
        if (mode == 0) {
            ...
            if ([[[NSBundle mainBundle] bundleIdentifier] isEqual:@"com.apple.springboard"] == NO) {
              CFNotificationCenterPostNotification(CFNotificationCenterGetDistributedCenter(), CFSTR("com.shade.protean/launchQR"), nil, (__bridge CFDictionaryRef)@{ @"appId":ident}, YES);
            } else {
              [Protean launchQR:ident];
            }
        } else {
          HBLogDebug(@"[Protean] invalid IconTap action: %d", mode);
        }
    }
    return nil;
}
...
static __attribute__((constructor)) void __protean_init() {
    ...
    if ([[[NSBundle mainBundle] bundleIdentifier] isEqual:@"com.apple.springboard"] == NO) {
        CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), NULL, &refreshStatusBar, CFSTR("com.shade.protean/refreshStatusBar"), NULL, 0);
    } else {
        CFNotificationCenterAddObserver(CFNotificationCenterGetDistributedCenter(), NULL, &launchQR, CFSTR("com.shade.protean/launchQR"), NULL, 0);
        CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), NULL, &refreshStatusBar, CFSTR("com.shade.protean/refreshStatusBar"), NULL, 0);
    }

    CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), NULL, &reloadSettings, CFSTR("com.shade.protean/reloadSettings"), NULL, 0);
